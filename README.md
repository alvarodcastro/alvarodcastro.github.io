...
The repository includes an automated workflow that periodically fetches your public ORCID works and creates new Markdown files for any publications not already present.

### How it works

1. **Cron schedule:** runs every Monday at 08:00 UTC (configurable in `.github/workflows/sync-orcid.yml`).
2. Calls `https://pub.orcid.org/v3.0/{ORCID_ID}/works` — no API key required for public records.
3. For each work not already present (matched by DOI or filename slug), fetches the full record and creates a `.md` file in `src/content/publications/`.
4. Opens a Pull Request so you can review and fill in missing fields before the change goes live.
...
| `OWNER_NAME` | Your name exactly as it appears in ORCID author lists, e.g. `Álvaro Researcher` |

**Step 2 — Grant Actions write permission**

Go to **Settings → Actions → General → Workflow permissions** and select **Read and write permissions**. This allows the workflow to push branches and open PRs.

**Step 3 — Trigger a first run (optional)**

Go to **Actions → Sync ORCID Publications → Run workflow**. Tick **Dry run** to see what would be created without writing anything.

### Generated file format

Each new file looks like this:
...
tags: []
featured: false
orcidPutCode: 12345678
---

<!-- Auto-generated by scripts/sync-orcid.mjs on 2025-07-01 -->
...
- **Contributor lists:** ORCID public records sometimes omit the full contributor list (especially for self-asserted works). The script falls back to your name only. Always check the author list in each PR.
- **Venue:** ORCID stores venue in the `journal-title` field, which is not always populated for conference papers. Fill it in manually if blank.
- **BibTeX:** Not available from ORCID. Add it manually after merging.
- **No deletion:** The script only adds files — it never removes or modifies existing ones.

### Running locally

```bash
ORCID_ID="0000-0000-0000-0000" OWNER_NAME="Your Name" node scripts/sync-orcid.mjs
# Dry run:
ORCID_ID="0000-0000-0000-0000" OWNER_NAME="Your Name" DRY_RUN=true node scripts/sync-orcid.mjs
```
